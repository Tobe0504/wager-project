...
const WagerInfo = () => {
  ...
  const formatAmount = (amount: string | null) => {
    ...
    const bnAmount =  new BN(amount.replace(/,/g, '')).div(new BN(10**decimals))
  ...
  const claimWin = async (wager: any) => {
    ...
    setBtnLoading(true)
    try {
      const result = await contractTx(api, activeAccount.address, contract, 'claimWin', {}, [
        wager.id
      ])
      setBtnLoading(false)
  ...
  const copyWagerLink = async() => {
    ...
    try {
      ...
      setMessage({ type: "success", message: "Wager link copied to clipboard" });
    } catch (err) {
  ...
  return (
    <section className={classes.container}>
      <div className={classes.textSection}>
        ...
        { wager.status === "Active" && 
          (wager.creator === activeAccount?.address || wager.bettor === activeAccount?.address) && 
          <div>
